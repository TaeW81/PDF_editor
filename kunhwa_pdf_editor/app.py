...
# Kunhwa PDF Editor v3.2 - 멀티창 + 교차 이동 + 사용자 관리 + 최적화 버전
VERSION = "v3.2"
...
def main():
    """Kunhwa PDF Editor 메인 진입점 (pip install 후 실행 지원)"""
    import argparse

    parser = argparse.ArgumentParser(
        description="Kunhwa PDF Editor v3.2 - PDF 편집 및 관리 프로그램"
    )
    parser.add_argument("file", nargs="?", default=None, help="열 PDF 파일 경로")
    parser.add_argument(
        "--install-shortcut",
        action="store_true",
        help="바탕화면 및 시작메뉴에 바로가기를 생성합니다",
    )
    ...
    if args.install_shortcut:
        from kunhwa_pdf_editor.install_shortcut import create_shortcuts
        create_shortcuts()
        return
    ...
    # 인증 성공 시 프로그램 실행
    def launch_new_editor(initial_path=None):
