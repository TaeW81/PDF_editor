...
# Kunhwa PDF Editor v2.6 - 멀티창 + 교차 이동 버전
VERSION = "v2.6"

# 허용된 사용자 맥어드레스 목록
ALLOWED_MAC_ADDRESSES = [
    "80-E8-2C-EF-97-E0",  # 장태웅
    "6C-0B-5E-42-ED-70",  # 최건호
    "80-E8-2C-EF-9E-4E",  # 문석환
    "E0-73-E7-BB-62-DE",  # 허재혁
    "6C-0B-5E-42-EC-3D",  # 유청산
    "BC-E9-2F-A1-AD-6C",  # 김지환
    "2C-58-B9-92-F5-CB",  # 김대영
    "80-E8-2C-EF-32-0D",  # 장소연
    "6C-0B-5E-42-EB-FE"   # 왕세환
]
...
def check_authorization():
    ...
    # 허용된 맥어드레스와 비교
    for allowed_mac in ALLOWED_MAC_ADDRESSES:
        if current_mac == allowed_mac:
            # 사용자 이름 찾기 (터미널 출력용)
            user_names = {
                "80-E8-2C-EF-97-E0": "장태웅",
                "6C-0B-5E-42-ED-70": "최건호",
            ...
            user_name = user_names.get(allowed_mac, "알 수 없음")
            print(f"인증 성공: {user_name}")
            return True
    
    # 인증 실패 시 오류 메시지 (개인정보 제거)
    messagebox.showerror("접근 거부", 
                        f"이 프로그램은 허용된 사용자만 실행할 수 있습니다.\n\n"
                        f"관리자에게 문의하세요.")
    return False
...
class PDFEditorApp:
    def __init__(self, root):
        ...
        for allowed_mac in ALLOWED_MAC_ADDRESSES:
            if current_mac == allowed_mac:
                user_name = "최건호" if allowed_mac == "6C-0B-5E-42-ED-70" else "장태웅"
                break
        ...
        # 페이지 클립보드 (교차 창 공유)
        self.page_clipboard = []  # [(source_doc, page_index)]는 사용하지 않고, 안전복사용 temp 문서 바이너리로 저장
        self.page_clipboard_bytes = None  # 여러 페이지를 포함하는 임시 PDF의 bytes
    ...
    def setup_menu_bar(self):
        ...
        edit_menu.add_command(label="페이지 회전 (우로90°)", accelerator="Ctrl+]", command=lambda: self.rotate_pages(90))
        edit_menu.add_command(label="페이지 회전 (좌로90°)", accelerator="Ctrl+[", command=lambda: self.rotate_pages(-90))
        edit_menu.add_command(label="빈페이지 삽입", command=self.show_insert_blank_page_dialog)
        edit_menu.add_separator()
        edit_menu.add_command(label="텍스트 추출", command=self.extract_text_directly)
        ...
        try:
            ...
            try:
                if hasattr(self, 'thumb_filename_label'):
                    self.thumb_filename_label.config(text=os.path.basename(file_path))
            except Exception:
                pass
    ...
    def setup_ui(self):
        ...
        ModernButton(pdf_frame, text="병합", command=self.merge_pdf, 
                    bg="#3B82F6", fg="white").pack(side=tk.LEFT, padx=1)
        ModernButton(pdf_frame, text="저장", command=self.save_pdf, 
                    bg="#1D4ED8", fg="white").pack(side=tk.LEFT, padx=1)
        ModernButton(pdf_frame, text="선택 저장", command=self.save_selected_pages, 
                    bg="#7C3AED", fg="white").pack(side=tk.LEFT, padx=1)
        # 새 창 버튼 (같은 프로세스 - DnD 호환)
        # (요청) 새 창 버튼은 파일 메뉴의 새 창(빈)과 동일 동작 사용
        ModernButton(pdf_frame, text="새 창", command=self.new_window,
        ...
        rotate_frame.pack(side=tk.LEFT, padx=4)
        
        ModernButton(rotate_frame, text="우로90°", command=lambda: self.rotate_pages(90), 
                    bg="#F59E0B", fg="white").pack(side=tk.LEFT, padx=1)
        ModernButton(rotate_frame, text="좌로90°", command=lambda: self.rotate_pages(-90), 
        ...
        ModernButton(top_frame, text="빈페이지", command=self.show_insert_blank_page_dialog, 
                    bg="#10B981", fg="white").pack(side=tk.LEFT, padx=3)
        ...
        text_frame.pack(side=tk.LEFT, padx=4)
        
        ModernButton(text_frame, text="텍스트", command=self.extract_text_directly, 
                    bg="#F59E0B", fg="white").pack(side=tk.LEFT, padx=1)
        
        # 페이지 맞춤 버튼을 제일 오른쪽에 배치 (최적화된 색상과 크기)
        ModernButton(top_frame, text="맞춤", command=self.fit_page_to_screen, 
                    bg="#6366F1", fg="white").pack(side=tk.RIGHT, padx=2)
        ...
        shortcuts_info = tk.Label(info_frame, 
                                text="Ctrl+클릭: 다중선택 | Shift+클릭: 범위선택 | Delete: 삭제 | Ctrl+A: 전체선택 | ESC: 선택해제 | Ctrl+휠(썸네일): 크기조정",
        ...
        copyright_text = f"© 2025 Kunhwa Engineering & Consulting {VERSION} | Developed by TaeWoong Jang | 인증된 사용자: {user_name}"
    ...
    def open_pdf(self):
        ...
        self.refresh_thumbnails()
        self.update_preview()
        # 썸네일 타이틀과 창 제목에 파일명 표시
        try:
            filename = os.path.basename(path)
            if hasattr(self, 'thumb_filename_label'):
                self.thumb_filename_label.config(text=filename)
            if hasattr(self, 'root') and self.root:
                self.root.title(f"Kunhwa PDF Editor - {filename}")
        except Exception:
            pass
    ...
    def refresh_thumbnails(self):
        ...
        for i in range(len(self.doc)):
            # 다양한 페이지 크기(A1~A4)를 고려한 썸네일 생성
            page = self.doc[i]
            page_rect = page.rect
            
            # 페이지 비율에 따른 썸네일 크기 조정 (v2.4 최적화)
            base_thumb_width = 110  # 기본 썸네일 너비
            base_thumb_height = 150  # 기본 썸네일 높이
            
            # thumb_scale을 적용한 실제 썸네일 크기
            max_thumb_width = base_thumb_width * self.thumb_scale / 0.20  # 기본 스케일 0.20 기준
            max_thumb_height = base_thumb_height * self.thumb_scale / 0.20
            
            # 페이지 비율 유지하면서 썸네일 크기 계산
            if page_rect.width > page_rect.height:  # 가로형 페이지
            ...
            if DRAG_DROP_AVAILABLE:
                try:
                    ...
                    # 공통 드롭 라우팅 (창간 이동/파일 병합 모두 처리)
                    label.dnd_bind('<<Drop>>', self.on_generic_drop)
                    # 교차 창 드래그 시작 등록
                    label.drag_source_register(1, DND_TEXT)
                    label.dnd_bind('<<DragInitCmd>>', lambda e, idx=i: ("copy", DND_TEXT, f"PDFTHUMB::{self.window_id}::{idx}"))
                except Exception:
                    pass
            ...
            if DRAG_DROP_AVAILABLE:
                try:
                    ...
                    num_label.dnd_bind('<<DragInitCmd>>', lambda e, idx=i: ("copy", DND_TEXT, f"PDFTHUMB::{self.window_id}::{idx}"))
                except Exception:
                    pass
    ...
    def ensure_scroll_region(self):
        ...
        try:
            ...
            if self.thumbnail_frames:
                ...
                thumb_width = sample_frame.winfo_reqwidth() + 20
                thumb_height = sample_frame.winfo_reqheight() + 20
    ...
    def show_save_format_dialog(self, selected_pages):
        ...
        # 제목
        title_label = tk.Label(dialog, text=f"선택된 {len(selected_pages)}개 페이지 저장", 
                              font=("맑은 고딕", 14, "bold"))
        ...
        info_label = tk.Label(dialog, text=f"페이지: {page_list}", 
        ...
        pdf_btn = ModernButton(button_frame, text="📄 PDF로 저장", 
                              ...
                              bg="#DC2626", fg="white", font=("맑은 고딕", 12, "bold"))
        pdf_btn.pack(pady=10, fill=tk.X, padx=20)
        
        # JPG로 저장 버튼
        jpg_btn = ModernButton(button_frame, text="🖼️ JPG로 저장", 
                              command=lambda: self.save_pages_as_jpg(selected_pages, dialog),
                              bg="#059669", fg="white", font=("맑은 고딕", 12, "bold"))
        jpg_btn.pack(pady=10, fill=tk.X, padx=20)
        
        # 취소 버튼
        cancel_btn = ModernButton(button_frame, text="취소", 
                                 command=dialog.destroy,
                                 bg="#6B7280", fg="white")
        cancel_btn.pack(pady=10, fill=tk.X, padx=20)
    ...
    def save_pages_as_jpg(self, selected_pages, dialog):
        ...
        try:
            ...
            filename_dialog.title("파일명 입력")
            filename_dialog.geometry("400x200")
            ...
            # 안내 메시지
            tk.Label(filename_dialog, text="JPG 파일명을 입력하세요", 
            ...
            tk.Label(filename_dialog, text=info_text, 
                    font=("맑은 고딕", 9), fg="#6B7280").pack(pady=(0, 15))
            
            # 파일명 입력 프레임
            input_frame = tk.Frame(filename_dialog)
            input_frame.pack(pady=10, padx=20, fill=tk.X)
            
            tk.Label(input_frame, text="파일명:", font=("맑은 고딕", 10)).pack(side=tk.LEFT)
            filename_entry = tk.Entry(input_frame, font=("맑은 고딕", 10), width=20)
            filename_entry.pack(side=tk.LEFT, padx=(10, 5), fill=tk.X, expand=True)
            tk.Label(input_frame, text=".jpg", font=("맑은 고딕", 10)).pack(side=tk.LEFT)
            ...
            # 버튼 프레임
            button_frame = tk.Frame(filename_dialog)
            button_frame.pack(pady=20)
...
if __name__ == "__main__":
    ...
    # 커맨드라인 인자로 넘어온 파일들을 각각 새 창으로 열기
    if len(sys.argv) > 1:
        for p in sys.argv[1:]:
            subprocess.Popen([sys.executable, __file__, p])
    else:
        launch_new_editor()