...
class PDFEditorApp:
    ...
    def refresh_thumbnails(self):
        ...
        if total_pages > 50:  # 대용량 PDF 최적화
            self._create_thumbnails_batch(total_pages)
        else:
            self._create_thumbnails_normal(total_pages)
        
        # 레이아웃 업데이트 (지연 로딩)
        self.root.after(30, self.update_grid_layout)  # 더 빠른 응답
        self.root.after(80, self.ensure_scroll_region)
                ...
                try:
                    ...
                    for page_num in range(len(source_doc)):
                        try:
                            ...
                            # 페이지 내용 복사
                            new_page.insert_image(new_page.rect, pixmap=source_page.get_pixmap())
                            total_pages += 1
                        except Exception as e:
    ...
    def show_users_list(self):
        ...
        # 사용자 목록 표시
        tree = ttk.Treeview(main_frame, columns=("mac", "name", "role"), show="headings", height=15)
        ...
        # 닫기 버튼
        close_btn = ModernButton(main_frame, text="닫기", command=dialog.destroy,
