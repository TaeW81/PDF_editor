...
# Kunhwa PDF Editor v2.6 - 멀티창 + 교차 드래그 버전
VERSION = "v2.6"
...
class PDFEditorApp:
    ...
    def setup_drag_drop(self):
        ...
        try:
            ...
            # 교차 창 드롭을 위한 텍스트 드롭 수신 등록
            self.root.drop_target_register(DND_TEXT)
            self.root.dnd_bind('<<Drop>>', self.on_interwindow_drop)
    ...
    def setup_ui(self):
        ...
        ModernButton(pdf_frame, text="PDF 열기", command=self.open_pdf, 
                    bg="#2563EB", fg="white").pack(side=tk.LEFT, padx=2)
        ModernButton(pdf_frame, text="PDF 병합", command=self.merge_pdf, 
                    bg="#3B82F6", fg="white").pack(side=tk.LEFT, padx=2)
        ModernButton(pdf_frame, text="PDF 저장", command=self.save_pdf, 
                    bg="#1D4ED8", fg="white").pack(side=tk.LEFT, padx=2)
        ModernButton(pdf_frame, text="선택 페이지 저장", command=self.save_selected_pages, 
                    bg="#7C3AED", fg="white").pack(side=tk.LEFT, padx=2)
        ...
        ModernButton(rotate_frame, text="↱ 우로90°회전", command=lambda: self.rotate_pages(90), 
                    bg="#F59E0B", fg="white").pack(side=tk.LEFT, padx=2)
        ModernButton(rotate_frame, text="↰ 좌로90°회전", command=lambda: self.rotate_pages(-90), 
                    bg="#D97706", fg="white").pack(side=tk.LEFT, padx=2)
        ...
        ModernButton(top_frame, text="빈페이지 삽입", command=self.show_insert_blank_page_dialog, 
                    bg="#10B981", fg="white").pack(side=tk.LEFT, padx=4)
        ...
        ModernButton(top_frame, text="페이지에 맞춤", command=self.fit_page_to_screen, 
                    bg="#6366F1", fg="white").pack(side=tk.RIGHT, padx=4)
        ...
        for i in range(len(self.doc)):
            ...
            if DRAG_DROP_AVAILABLE:
                try:
                    ...
                    # 교차 창 드래그 시작 등록 (텍스트로 페이지 인덱스 전달)
                    label.drag_source_register(1, DND_TEXT)
                    label.dnd_bind('<<DragInitCmd>>', lambda e, idx=i: (('text/plain', 1, f"PDFTHUMB::{idx}"), tk.DND_REFERENCE))
                except:
                    pass
            ...
            if DRAG_DROP_AVAILABLE:
                try:
                    ...
                    num_label.dnd_bind('<<DragInitCmd>>', lambda e, idx=i: (('text/plain', 1, f"PDFTHUMB::{idx}"), tk.DND_REFERENCE))
                except:
                    pass
    ...
    def on_interwindow_drop(self, event):
        ...
        try:
            ...
            # 예: 'PDFTHUMB::12'
            try:
                src_index = int(data.split('PDFTHUMB::')[-1])
            except Exception:
                return
            ...
            for app in OPEN_EDITORS:
                if app is not self and app.doc is not None and src_index < len(app.doc):
                    src_app = app
                    break
            if src_app is None:
                return
            ...
            # 소스 페이지를 픽스맵으로 복사해 새 페이지에 삽입
            page = src_app.doc[src_index]
            new_page = self.doc.new_page(width=page.rect.width, height=page.rect.height)
            new_page.insert_image(new_page.rect, pixmap=page.get_pixmap())
            # 새 페이지를 지정 위치로 이동
            try:
    ...
    def save_pages_as_jpg(self, selected_pages, dialog):
        ...
        try:
            # 저장 폴더 선택
            folder_path = filedialog.askdirectory(title="JPG 파일들을 저장할 폴더 선택")
