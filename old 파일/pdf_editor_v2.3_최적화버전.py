...
class PDFEditorApp:
    ...
    def setup_thumbnail_panel(self):
        ...
        self.thumb_canvas.pack(side=tk.LEFT, fill=tk.BOTH, expand=True, padx=10, pady=5)
    ...
    def refresh_thumbnails(self):
        ...
        for i in range(len(self.doc)):
            ...
            # 페이지 비율에 따른 썸네일 크기 조정
            max_thumb_width = 150  # 최대 썸네일 너비
            max_thumb_height = 200  # 최대 썸네일 높이
    ...
    def update_grid_layout(self):
        ...
        if self.thumbnail_frames:
            ...
            # 다양한 페이지 크기(A1~A4)를 고려한 안전한 여백 설정
            # A1 페이지가 가장 크므로 충분한 여백 확보
            safety_margin = 50  # 좌우 각각 25px씩 추가 여백
            
            # 사용 가능한 열 수 계산 (안전 여백 포함)
            available_width = canvas_width - safety_margin
            col_count = max(1, available_width // thumb_width)
            
            # 열 수가 너무 많으면 줄이기 (썸네일이 잘리지 않도록)
            if col_count * thumb_width > available_width:
            ...
            # 전체 그리드 크기 계산 (안전 여백 포함)
            total_width = max(col_count * thumb_width + safety_margin, canvas_width)
            total_height = row_count * thumb_height
            
            # 스크롤 영역 설정 (여백 포함) - 하단에 2열 정도 여유 공간 추가
            extra_height = thumb_height * 2 + 20  # 2열 + 패딩
            
            # 스크롤 영역 설정 (우측 여유 공간 포함)
    ...
    def ensure_scroll_region(self):
        ...
        try:
            ...
            if self.thumbnail_frames:
                ...
                thumb_width = sample_frame.winfo_reqwidth() + 10
                thumb_height = sample_frame.winfo_reqheight() + 10
                
                # 다양한 페이지 크기를 고려한 안전 여백
                safety_margin = 50  # 좌우 각각 25px씩
                
                # 2열의 높이만큼 여유 공간 계산
                extra_height = thumb_height * 2 + 20
                
                # 스크롤 영역 계산 - 우측 안전 여백과 하단 여유 공간 포함
                scroll_width = max(current_scroll[2] + safety_margin, canvas_width)
