...
class ModernButton(tk.Button):
    ...
    def apply_normal_style(self):
        ...
        elif '우수흐름 방향 표시' in self.cget('text'):
            self.config(bg='#9C27B0', fg='white')
        else:
...
class PDFEditorApp:
    ...
    def setup_ui(self):
        ...
        # 우수흐름 방향 표시 버튼 추가
        ModernButton(center_buttons, text="우수흐름 방향 표시", command=self.show_flow_direction_info, width=15, height=2, tooltip="CDS 공평 출력 후, 도로 중심선과 FH(TEXT)를 선택하면\nFH 고도 차이를 기준으로 우수 흐름 방향을 화살표로 표시하여\n우수 초기계획 방향을 파악할 수 있습니다.").pack(side=tk.LEFT, padx=(5, 0))
        ...
        self.preview_frame = tk.Frame(self.content_frame, bg="gray90")
        ...
        # 선택 상태 색상 설정
        self.selection_color = '#FF5722'
        self.selection_bg = '#FFCDD2'
        self.selection_highlight = '#FF5722'
        
        # 썸네일 그리드 레이아웃 업데이트 예약
    ...
    def create_thumbnail(self, page_index):
        ...
        try:
            ...
            # 썸네일 크기 조정
            thumb_width = int(150 * self.thumb_scale)
            thumb_height = int(200 * self.thumb_scale)
            img = img.resize((thumb_width, thumb_height), Image.Resampling.LANCZOS)
            
            photo = ImageTk.PhotoImage(img)
            
            # 썸네일 프레임 생성
            frame = tk.Frame(self.scrollable_frame, relief=tk.RAISED, bd=2)
            frame.pack(fill=tk.X, padx=5, pady=2)
            
            # 썸네일 이미지
            label = tk.Label(frame, image=photo, text=f"페이지 {page_index + 1}", compound=tk.TOP)
            label.pack(padx=5, pady=5)
            label.image = photo
            
            # 이벤트 바인딩
            label.bind('<Button-1>', lambda e, idx=page_index: self.handle_selection(idx, False))
            label.bind('<Double-Button-1>', lambda e, idx=page_index: self.on_double_click(idx))
            label.bind('<B1-Motion>', lambda e, idx=page_index: self.on_drag(e, idx))
            label.bind('<ButtonRelease-1>', lambda e, idx=page_index: self.on_drop(e, idx))
            label.bind('<Enter>', lambda e, f=frame: self.on_enter(f))
            label.bind('<Leave>', lambda e, f=frame: self.on_leave(f))
    ...
    def handle_selection(self, index, clear_others=False):
        """선택 처리"""
        if clear_others:
            self.selected_indices.clear()
    ...
    def update_selection_highlight(self):
        ...
        for i, frame in enumerate(self.thumbnail_frames):
            if i in self.selected_indices:
                frame.config(bg=self.selection_bg, highlightbackground=self.selection_highlight, highlightthickness=2)
            else:
                frame.config(bg=self.default_frame_bg, highlightbackground="", highlightthickness=0)
    
    def on_enter(self, frame):
        """마우스 진입"""
        if frame not in [self.thumbnail_frames[i] for i in self.selected_indices]:
            frame.config(bg='#E3F2FD')
    
    def on_leave(self, frame):
        """마우스 이탈"""
        if frame not in [self.thumbnail_frames[i] for i in self.selected_indices]:
            frame.config(bg=self.default_frame_bg)
    ...
    def update_grid_layout(self):
        ...
        frame_req_w = self.thumbnail_frames[0].winfo_reqwidth()
        canvas_width = self.canvas.winfo_width()
        
        if frame_req_w < 20 or canvas_width < 20:
            self.root.after(50, self.update_grid_layout)
            return
        
        sample_width = frame_req_w + 10
    ...
    def on_thumbnail_scroll(self, event):
        ...
        if event.state & 0x4:  # Ctrl 키가 눌린 상태
            # 줌
            if event.delta > 0:
                self.thumb_scale = min(self.thumb_scale * 1.1, 3.0)
            else:
                self.thumb_scale = max(self.thumb_scale * 0.9, 0.3)
            self.refresh_thumbnails()
        else:
            # 스크롤
    ...
    def show_flow_direction_info(self):
        """우수흐름 방향 표시 기능 정보 팝업"""
        popup = tk.Toplevel(self.root)
        popup.title("우수흐름 방향 표시 기능")
        ...
        # 설명 텍스트
        description_text = """CDS 공평 출력 후, 도로 중심선과 FH(TEXT)를 선택하면
FH 고도 차이를 기준으로 우수 흐름 방향을 화살표로 표시하여
우수 초기계획 방향을 파악할 수 있습니다.
...
【 사용 방법 】
1. CDS에서 공평 출력
2. 도로 중심선 선택 (레이어: 도로중심선)
3. FH 텍스트 선택 (레이어: 도로체인계획고)
4. 우수흐름 방향 표시 실행
5. 자동으로 화살표 생성
...
• 색상: 보라색 계열"""
    ...
    def on_drop_files(self, event):
        ...
        try:
            ...
            print(f"드롭 위치 계산: x={event.x_root}, y={event.y_root}")
            print(f"삽입 위치: {drop_index}")
            ...
            for pdf_path in pdf_files:
                try:
                    print(f"병합 시도: {pdf_path}")
                    added = self.merge_external_pdf(pdf_path, running_index)
                    if added > 0:
                        running_index += added
                        success_count += 1
                        print(f"병합 성공: {pdf_path}, 추가된 페이지: {added}")
                    else:
                        print(f"병합 실패: {pdf_path}")
                except Exception as e:
    ...
    def get_drop_index_from_position(self, x, y):
        ...
        try:
            ...
            print(f"왼쪽 프레임 위치: x={left_frame_x}, y={left_frame_y}, w={left_frame_w}, h={left_frame_h}")
            
            # 마우스가 왼쪽 썸네일 영역에 있는지 확인
            if (x < left_frame_x or x > left_frame_x + left_frame_w or 
                y < left_frame_y or y > left_frame_y + left_frame_h):
                print(f"마우스가 썸네일 영역 밖에 있음")
                return None
            
            print(f"마우스가 썸네일 영역 안에 있음")
            ...
            for i, frame in enumerate(self.thumbnail_frames):
                try:
                    ...
                    print(f"썸네일 {i+1} 위치: x={frame_x}, y={frame_y}, w={frame_w}, h={frame_h}")
                    
                    # 마우스가 이 썸네일 위에 있는지 확인
                    if (frame_x <= x <= frame_x + frame_w and 
                        frame_y <= y <= frame_y + frame_h):
                        print(f"마우스가 썸네일 {i+1} 위에 있음 -> 삽입 위치: {i}")
                        return i
                    ...
                    if i > 0:
                        ...
                        if (prev_frame_bottom <= y <= frame_y and 
                            ...
                            print(f"마우스가 썸네일 {i}과 {i+1} 사이에 있음 -> 삽입 위치: {i}")
                            return i
                    
                except Exception as e:
                    print(f"썸네일 {i+1} 위치 계산 오류: {e}")
                    continue
            ...
            if self.thumbnail_frames:
                ...
                try:
                    ...
                    if y > last_frame_bottom:
                        print(f"마우스가 마지막 썸네일 아래에 있음 -> 문서 끝에 추가")
                        return len(self.doc)
                except Exception as e:
                    print(f"마지막 썸네일 위치 계산 오류: {e}")
            ...
            if self.thumbnail_frames:
                ...
                try:
                    ...
                    if y < first_frame_top:
                        print(f"마우스가 첫 번째 썸네일 위에 있음 -> 첫 번째 위치에 추가")
                        return 0
                except Exception as e:
                    print(f"첫 번째 썸네일 위치 계산 오류: {e}")
            
            # 기본값은 첫 번째 위치
            print(f"기본값 사용 -> 첫 번째 위치에 추가")
            return 0
