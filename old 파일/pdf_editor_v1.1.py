...
class PDFEditorApp:
    ...
    def on_drop_file(self, event):
        ...
        try:
            ...
            if hasattr(self, 'drop_target_index'):
                ...
                # 일반 드롭 처리
                drop_target = self.get_drop_target_from_event(event)
                print(f"일반 드롭: 타겟 위치 {drop_target}")
            ...
            #     messagebox.showinfo("완료", f"{len(supported_files)}개 파일이 병합되었습니다.")
    ...
    def get_drop_target_from_event(self, event):
        ...
        try:
            ...
            if widget == self.thumb_canvas:
                ...
                return self.get_thumbnail_drop_position(event)
        ...
        try:
            ...
            if self.thumbnail_frames:
                ...
                if 0 <= index < len(self.thumbnail_frames):
                    ...
                    return index
                elif index >= len(self.thumbnail_frames):
                    # 마지막 위치로 이동
                    print(f"드롭 위치 계산: 마지막 위치 {len(self.thumbnail_frames)}")
                    return len(self.thumbnail_frames)
                else:
                    print(f"드롭 위치 계산: 기본 위치 0")
                    return 0
    ...
    def setup_ui(self):
        ...
        self.selection_info = tk.Label(info_frame, text="선택된 페이지: 없음", 
                                     bg="white", fg="#212529", font=("Segoe UI", 11, "bold"))
        ...
        shortcuts_info = tk.Label(info_frame, 
                                ...
                                bg="white", fg="#495057", font=("Segoe UI", 9, "bold"))
        ...
        copyright_label = tk.Label(copyright_frame, 
                                 ...
                                 bg="white", fg="#495057", font=("Segoe UI", 8, "bold"))
    ...
    def setup_thumbnail_panel(self):
        ...
        title_label = tk.Label(title_frame, text="페이지 썸네일", 
                               bg='#f8f9fa', fg='#212529', font=("Segoe UI", 12, "bold"))
    ...
    def on_preview_mousewheel(self, event):
        ...
        # Ctrl 키가 눌려있으면 페이지 넘김 방지 (줌만 허용)
        if event.state & 0x0004:  # Ctrl 키가 눌려있음
            return
        ...
        if self.is_page_fit_to_screen():
            ...
            # 페이지가 화면보다 큰 경우, 일반 스크롤
            # 스크롤이 페이지 끝에 도달했는지 확인
            if self.is_scroll_at_page_boundary(event.delta):
                # 페이지 끝에 도달한 경우, 페이지 이동
                if event.delta > 0:
                    # 위로 스크롤 - 이전 페이지
                    self.go_to_previous_page()
                else:
    ...
    def is_scroll_at_page_boundary(self, delta):
        """스크롤이 페이지 끝에 도달했는지 확인"""
        if not self.doc or self.current_page_index >= len(self.doc):
            return False
        ...
        try:
            ...
            scroll_top = current_scroll[0]  # 현재 스크롤 위치 (0.0 ~ 1.0)
            scroll_bottom = current_scroll[1]  # 현재 보이는 영역의 끝 (0.0 ~ 1.0)
            
            # 페이지 크기와 캔버스 크기 가져오기
            page = self.doc[self.current_page_index]
            page_height = page.rect.height * self.preview_scale
            canvas_height = self.preview_canvas.winfo_height()
            ...
            # 스크롤 가능한 전체 높이
            total_height = max(page_height, canvas_height)
            
            # 스크롤 위치를 픽셀 단위로 변환
            current_y = scroll_top * total_height
            visible_height = (scroll_bottom - scroll_top) * total_height
            
            # 위로 스크롤할 때 (delta > 0) - 페이지 상단에 도달했는지 확인
            if delta > 0:
                return current_y <= 10  # 상단에서 10px 이내
            # 아래로 스크롤할 때 (delta < 0) - 페이지 하단에 도달했는지 확인
            else:
                return current_y + visible_height >= page_height - 10  # 하단에서 10px 이내
            
        except Exception as e:
            print(f"스크롤 경계 확인 중 오류: {e}")
            return False
    ...
    def refresh_thumbnails(self):
        ...
        for i in range(len(self.doc)):
            ...
            num_label = tk.Label(frame, text=f"{i+1}", font=("Segoe UI", 10, "bold"), 
                               bg="white", fg="#212529")
        ...
        try:
            if (abs(event.x - self.drag_data["x"]) > 5 or abs(event.y - self.drag_data["y"]) > 5):
                ...
                if drop_target is not None:
                    ...
                    # 선택된 페이지 전체 이동
                    if messagebox.askyesno("페이지 이동", f"선택된 {len(self.selected_indices)}개 페이지를 위치 {drop_target+1}로 이동하시겠습니까?"):
    ...
    def show_insert_blank_page_dialog(self):
        ...
        # 페이지 크기 선택
        tk.Label(dialog, text="페이지 크기를 선택하세요:", font=("Arial", 12)).pack(pady=10)
