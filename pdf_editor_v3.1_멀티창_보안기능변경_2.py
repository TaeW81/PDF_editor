...
# Kunhwa PDF Editor v3.1 - 멀티창 + 교차 이동 + 사용자 관리 + 최적화 버전
VERSION = "v3.1"

# 사용자 정보는 외부 암호화 파일(users.json.enc)에서 관리
# 더 이상 코드 내부에 하드코딩하지 않음
...
def check_authorization():
    ...
    # 파일이 없으면 기본 파일 생성
    if not users_data:
        print("사용자 정보 파일이 없습니다. 기본 파일을 생성합니다.")
        if create_default_users_file():
            users_data = load_encrypted_users()
        else:
            print("기본 파일 생성 실패. 하드코딩된 목록 사용")
            users_data = None
    ...
    if users_data and "users" in users_data:
        ...
        # 외부 파일이 없거나 손상된 경우
        print("사용자 정보 파일이 없거나 손상되었습니다.")
        messagebox.showerror("인증 오류", 
                            "사용자 정보 파일을 찾을 수 없습니다.\n"
                            "관리자에게 문의하세요.")
        return False
...
class PDFEditorApp:
    ...
    def _update_user_menu_visibility(self):
        """사용자 권한에 따라 메뉴 가시성 업데이트"""
        try:
            # 현재 사용자의 맥어드레스 가져오기
            current_mac = get_mac_address()
            if not current_mac:
                # 맥어드레스를 확인할 수 없으면 메뉴 비활성화
                self.users_menu.entryconfig("사용자 목록 보기", state="disabled")
                self.users_menu.entryconfig("사용자 추가", state="disabled")
                self.users_menu.entryconfig("사용자 제거", state="disabled")
                self.users_menu.entryconfig("JSON 파일 편집", state="disabled")
                self.users_menu.entryconfig("사용자 백업", state="disabled")
                self.users_menu.entryconfig("백업 복원", state="disabled")
                return
            
            # 사용자 데이터에서 현재 사용자 정보 찾기
            users_data = load_encrypted_users()
            if not users_data:
                # 사용자 정보를 불러올 수 없으면 메뉴 비활성화
                self.users_menu.entryconfig("사용자 목록 보기", state="disabled")
                self.users_menu.entryconfig("사용자 추가", state="disabled")
                self.users_menu.entryconfig("사용자 제거", state="disabled")
                self.users_menu.entryconfig("JSON 파일 편집", state="disabled")
            ...
            if current_user["role"] == "admin":
                # 관리자면 모든 메뉴 활성화
                self.users_menu.entryconfig("사용자 목록 보기", state="normal")
                self.users_menu.entryconfig("사용자 추가", state="normal")
                self.users_menu.entryconfig("사용자 제거", state="normal")
                self.users_menu.entryconfig("JSON 파일 편집", state="normal")
                self.users_menu.entryconfig("사용자 백업", state="normal")
                self.users_menu.entryconfig("백업 복원", state="normal")
            else:
                # 일반 사용자면 모든 메뉴 비활성화
                self.users_menu.entryconfig("사용자 목록 보기", state="disabled")
                self.users_menu.entryconfig("사용자 추가", state="disabled")
    ...
    def update_grid_layout(self):
        """썸네일을 창 크기에 맞춰 자동으로 배열하는 그리드 레이아웃 (v2.8 안정 버전)"""
    ...
    def ensure_scroll_region(self):
        """스크롤 영역이 제대로 설정되었는지 확인하고 필요시 수정 (v2.8 안정 버전)"""
        if not self.doc or not self.thumbnail_frames:
            return
        ...
        try:
            ...
            if self.thumbnail_frames:
                ...
                # 행 수 계산
                total_pages = len(self.thumbnail_frames)
                row_count = (total_pages + col_count - 1) // col_count
                
                # 전체 그리드 크기 계산
                total_width = col_count * thumb_width + total_margin
                total_height = row_count * thumb_height + 40
                
                # 스크롤 영역 설정
                self.thumb_canvas.config(scrollregion=(0, 0, total_width, total_height))
                
                print(f"스크롤 영역 업데이트: {col_count}열 x {row_count}행, 크기: {total_width}x{total_height}")
                
        except Exception as e:
            print(f"스크롤 영역 업데이트 오류: {e}")
    ...
    def _create_thumbnails_normal(self, total_pages):
        """일반 썸네일 생성 (50페이지 이하)"""
        for i in range(total_pages):
            self._create_single_thumbnail(i)

    def _create_thumbnails_batch(self, total_pages):
        """배치 썸네일 생성 (50페이지 초과, 지연 로딩)"""
        def create_batch(start_idx, batch_size=10):
            ...
            if end_idx < total_pages:
                self.root.after(50, lambda: create_batch(end_idx, batch_size))
    ...
    def _create_single_thumbnail(self, index):
        ...
        try:
            ...
            if cache_key in self._thumbnail_cache:
                ...
                self._create_thumbnail_widget(index, cached_data['image'])
                return
            ...
            # 위젯 생성
            self._create_thumbnail_widget(index, thumb)
...
프로그램을 재시작하면 변경사항이 적용됩니다.
    ...
    def _remove_user_from_file(self, index):
        ...
        try:
            ...
            # 사용자 제거
            removed_user = users_data["users"].pop(index)
            users_data["last_updated"] = "2025-01-27"  # 현재 날짜로 업데이트
