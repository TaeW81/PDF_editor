...
- 사용자 관리 메뉴에서 팀원 권한을 관리하세요.
    ...
    def create_menu(self):
        ...
        file_menu.add_command(label="인쇄", command=self.on_print, accelerator="Ctrl+P")
    ...
    def create_toolbar_buttons(self):
        ...
        ttk.Button(grp_file, text="인쇄", command=self.on_print, bootstyle="dark").pack(side=LEFT, padx=2)
    ...
    def on_print(self):
        """Print the current PDF using system dialog."""
        if not self.pdf.file_path:
             messagebox.showwarning("경고", "인쇄할 PDF를 열어주세요.")
             return
             
        try:
            # Try system print command using os.startfile
            os.startfile(self.pdf.file_path, "print")
        except OSError as e:
            # WinError 1155: No application associated with the specified file for this operation
            if e.winerror == 1155:
                # Auto-fallback: Open the file
                try:
                    os.startfile(self.pdf.file_path)
                    messagebox.showinfo("인쇄 안내", "자동 인쇄(Print) 기능이 윈도우에 연결되어 있지 않습니다.\n\n파일을 열어드렸으니 뷰어에서 인쇄(Ctrl+P)해주세요.")
                except Exception as e2:
                    messagebox.showerror("오류", f"파일 열기 실패: {e2}")
            else:
                 # Other errors
                 messagebox.showerror("인쇄 오류", f"인쇄 명령 실패: {e}")
        except Exception as e:
             messagebox.showerror("인쇄 오류", f"알 수 없는 오류: {e}")
